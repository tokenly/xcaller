  ...
  processJob = function(job, callback) {
    ...
    finishJob = function(success, err) {
      ...
      if (finished) {
        ...
        insertJobIntoBeanstalk('notifications_return', queueEntry, 10, 0, function(loadSuccess) {
          if (loadSuccess) {
            callback(true);
          }
        });
      } else {
        insertJobIntoBeanstalk('notifications_out', jobData, RETRY_PRIORITY, RETRY_DELAY * jobData.meta.attempt, function(loadSuccess) {
          if (loadSuccess) {
            callback(true);
          }
        });
